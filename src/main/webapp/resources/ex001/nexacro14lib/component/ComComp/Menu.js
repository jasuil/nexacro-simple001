...
if (!nexacro.Menu) {
	...
	_pMenu.on_notify_menuitem_onmouseleave = function (obj, e) {
		...
		if (this._find_lastFocused() == this._getRootComponent(this)) {
			this._on_focus(true);
		}
	};
	...
	_pMenu._showPopup = function (obj, x, y, bcapture) {
		...
		if (this._innerdataset && this.levelcolumn && this.captioncolumn && this.idcolumn && obj.itemexpand == true) {
			var popupmenu = this._popupmenu;
	...
	_pMenu._killfocus_item = function () {
		...
		if (items) {
			this._is_menu_click = false;
			this._clickitemindex = -1;
			if (items[this._menuitemindex]) {
				items[this._menuitemindex]._killfocus_item();
			}
			this._menuitemindex = 0;
		}
	};

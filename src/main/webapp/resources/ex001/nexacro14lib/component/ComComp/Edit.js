...
if (!nexacro.Edit) {
	...
	_pEdit.on_apply_custom_setfocus = function (evt_name) {
		...
		if (input_elem) {
			...
			var text = this.text;
			if ((this._old_selection[0] != this._old_selection[1]) || this._is_set_value) {
			...
			if (!this.autoselect) {
				if (api && evt_name != "tabkey" && evt_name != "shifttabkey") {
					this.setCaretPos(api._begin_pos);
				}
				else if (nexacro.Browser == "Gecko" && this._is_set_value) {
	...
	_pEdit._on_fire_textchangeEventSet = function (text_info) {
		...
		if (ret) {
			...
			if (org_chartext != changechartext) {
				...
				if (!api._is_hangul(org_chartext) && !api._is_half_japaness(org_chartext) && !api._is_english(org_chartext)) {
					strFront = text_info.pretext.substr(0, api._begin_pos - org_chartext.length);
				}
				else if (api._is_english(org_chartext) && api._stat_composition.getCurrentStatus() == nexacro.EditBase.Status.CompositionEnd) {
					strFront = text_info.pretext.substr(0, api._begin_pos - org_chartext.length);
				}
				else {

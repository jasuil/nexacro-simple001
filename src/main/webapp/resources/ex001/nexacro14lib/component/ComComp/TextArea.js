...
if (!nexacro.TextArea) {
	...
	_pTextArea.on_apply_custom_setfocus = function (evt_name) {
		...
		if (input_elem) {
			...
			if ((this._old_selection[0] != this._old_selection[1]) || (nexacro.Browser != "Edge" && this._is_set_value)) {
			...
			if (nexacro.Browser == "Runtime" && nexacro.OS == "Windows" && !this._is_set_value) {
				...
				if (!this.autoselect && api && evt_name != "tabkey" && evt_name != "shifttabkey" && nexacro.Browser != "IE") {
					this.setCaretPos(api._begin_pos);
				}

				if (this._is_set_value && (evt_name == "tabkey" || evt_name == "shifttabkey")) {
					if (nexacro.Browser == "Chrome" || nexacro.Browser == "Opera") {
						...
						if (this.vscrollbar) {
							var pos = input_elem.getScrollHeight();
							this.vscrollbar.set_pos(pos);
						}

						if (this.hscrollbar) {
							var pos = input_elem.getScrollWidth();
							this.hscrollbar.set_pos(pos);
						}
					}
				}
				...
				nexacro.OnceCallbackTimer.callonce(pThis, function () {
					...
					if (nexacro.Browser == "Gecko" && this._is_first_focus) {
						this._is_first_focus = false;
						input_elem.setElementSetSelect(api._begin_pos, api._end_pos);
					}
				});
	...
	_pTextArea.set_value = function (v) {
		...
		if (v != this.value) {
			...
			if (this.value) {
				...
				if (nexacro.Browser != "IE") {
					this._old_selection[0] = this._old_selection[1] = this.value.length;
				}
			}
		}
	};
	...
	_pTextArea._cancelSelect = function () {
		...
		else if (nexacro.Browser == "Chrome" || nexacro.Browser == "Gecko" || nexacro.Browser == "Safari") {
			if (!api._is_selected()) {
				this.setCaretPos(api._begin_pos);
				this._setcaret = false;
			}
		...
		if (ret) {
			...
			if (!api._is_composition()) {
				if (nexacro.Browser == "IE" && nexacro.BrowserVersion == 11 || nexacro.Browser == "Edge") {
					...
					if (api._keycode == nexacro.KeyCode_ImeInput && api._stat_composition._curr_data == "" && imetext != "" && api._is_hangul(imetext)) {
						ret = false;
					}
					...
					if (api._keycode == nexacro.KeyCode_ImeInput && org_chartext == "") {
						ret = fireCancharEvent = false;
					}
				}

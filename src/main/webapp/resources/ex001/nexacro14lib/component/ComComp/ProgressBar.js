...
if (!nexacro.ProgressBar) {
	...
	_pProgressBar.on_apply_style_smooth = function (smooth) {
		...
		if (smooth == true && this.celllist && this.celllist.length > 1) {
			var celllist_len = this.celllist.length;
			for (i = 1; i < celllist_len; i++) {
	...
	_pProgressBar.on_create_contents = function () {
		...
		if (control_elem) {
			...
			if (curBartype != "image") {
				...
				this.cellitem._setRtlDirection(this._rtldirection);
			}
		}
	};
		...
		if (smooth == "true") {
			...
			if (direction == "backward") {
				...
				for (var i = size; i > 0; i--) {
					...
					cell_list[i]._setRtlDirection(this._rtldirection);
				}
	...
	_pProgressBar._create_img = function (img, img_val, id) {
		if (img_val != null && img_val != "") {
			...
			img.createComponent();
			img._setRtlDirection(this._rtldirection);
			if (id == "startImg") {

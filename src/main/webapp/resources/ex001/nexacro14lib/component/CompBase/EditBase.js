...
if (!nexacro.EditBase) {
	...
	if (nexacro.Browser == "IE" && nexacro.BrowserVersion <= 10 && nexacro._getRealBrowserVersion() <= 10) {
		...
		if (major_ver < 7) {
			...
			_pEditBase._on_default_input_keyinput = function (elem) {
				...
				if (this._charcode >= 12593 && this._charcode <= 12643) {
				...
				else if (this._is_full_japaness(this._precharcode) && this._charcode == 0 && (major_ver >= 10 && minor_ver >= 3)) {
					...
					if (this._is_full_japaness(this._precharcode) && this._keycode != 8 && this._charcode == 0 && !(major_ver >= 10 && minor_ver >= 3)) {
						if (this.getCompositionData(elem) == "") {
							return false;
						}
					}
		...
		_pEditBase._on_default_input_keyinput = function (elem) {
			...
			if (nexacro.OS == "Android" && nexacro.Browser == "Chrome") {
				if (comp.maxlength > 0 && elem_value.length > comp.maxlength && editbase_text.length >= comp.maxlength && this._select_text == "") {
					...
					if (this._is_composition()) {
						if (this._stat_composition._curr_stat == nexacro.EditBase.Status.CompositionUpdate) {
							...
							this._input_begin_pos = this._mobile_elme_pos = (elem_pos.begin > comp.maxlength) ? comp.maxlength : elem_pos.begin;
	...
	if (nexacro.Browser == "Edge" || (nexacro.Browser == "IE" && (nexacro.BrowserVersion == 11 || (nexacro.BrowserType == "IE10" && nexacro._getRealBrowserVersion() == 11)))) {
		...
		if (major_ver[0] >= 10 && major_ver[1] > 0) {
			_pEditBase._on_default_input_compositionend = function (data) {
				this._compositionstart_value = "";
				this._stat_composition.setStatus(nexacro.EditBase.Status.CompositionEnd, data);
	...
	_pEditNormal._on_default_input_keydown = function (elem, keyCode, altkey, ctrlkey, shiftkey) {
		...
		if (keyCode == nexacro.KeyCode_ImeInput) {
			...
			if (this._is_composition() && (keyCode == 8 && nexacro.Browser != "MobileSafari" || keyCode == 46 || (nexacro.OS != "iOS" && keyCode == 32) || (keyCode >= 33 && keyCode <= 40))) {
	...
	_pEditNormal._on_default_input_keypress = function (elem, keyCode, charCode, altKey, ctrlKey, shiftKey) {
		...
		if (nexacro.OS == "iOS" && (k == nexacro.Event.KEY_ENTER)) {
		...
		if (k == nexacro.Event.KEY_BACKSPACE || (k == nexacro.Event.KEY_DELETE && k == this._keycode) || (nexacro.Browser == "MobileSafari" && k == 127)) {
			...
			bFireEvt = ret1;
			if (nexacro.OS == "iOS" && nexacro.Browser == "MobileSafari" && ret1) {
				this._accept_keyinput_event = false;
			}
		}
		...
		if (bPrevent) {
			if (nexacro.OS == "iOS" && nexacro.Browser == "MobileSafari" && !this._accept_keyinput_event) {
				this._accept_keyinput_event = true;
			}
			elem._event_stop = true;
		}

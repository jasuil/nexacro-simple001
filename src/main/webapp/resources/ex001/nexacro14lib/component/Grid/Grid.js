...
if (!nexacro.Grid) {
	...
	_pGridCell._getClickItem = function (halign, valign, canvasX, canvasY, text, comp) {
		...
		var halign, valign;
		var include = false;
		var paddingleft = comp.currentstyle.padding.left;
		var paddingtop = comp.currentstyle.padding.top;
	...
	_pGridCell._hideEditor = function () {
		...
		if (this._subComp) {
			if (this._subComp._displaymode && !this._subComp._is_dblclick) {
				...
				if (nexacro.Browser == "Runtime" && this._subComp.setCaretPos) {
					this._subComp.setCaretPos(0);
				}
		...
		if (this._isBody) {
			...
			if (grid._is_created == true) {
				...
				if (grid._is_use_suppress) {
					var cells = grid._curFormat._bodycells, cells_cnt = cells ? cells.length : 0;
					var rowidx, datarow;
		...
		if (cellinfo) {
			...
			if (strPropID == "displaytype" && varValue == "tree") {
				...
				if (strPropID == "displaytype" || strPropID == "editdisplay" || strPropID == "combodisplay"
					 || strPropID == "calendardisplay" || strPropID == "edittype") {
	...
	_pGrid._selectRow = function (row, bSelect, noDraw, cell, bDataset) {
		...
		var retn = false;
		if (bSelect != this._getSelect(row)) {
			if (row == -1) {
				band = "head";
			}
			else if (row == -2) {
	...
	_pGrid._resizeBand = function (reset) {
		...
		if (bodyrowscnt !== undefined && bodyrowscnt == 0) {
			clientheight = this._client_height;
			if (this.summarytype == "top" || this.summarytype == "lefttop") {
				t = headHeight + summHeight;
				h = clientheight - t;
	...
	_pGrid._moveCellAfterFocus = function () {
		...
		if (this.autoenter == "select") {
			...
			if (this._currentBand == "body" && !this._showEditing && !this._showEditor(false, true)) {
				this._hideEditor();
				retn = false;
			}
			this._onceTime_focus = false;
			...
			for (var i = 0, len = elements.length; i < len; i++) {
				...
				if (sub_elements && sub_elements.length > 0) {
					...
					if (!(start_cell instanceof nexacro.GridCell)) {
						start_cell = sub_elements[1].linkedcontrol;
					}
	...
	_pGrid._destroySelectBorderElements = function (start_index) {
		...
		if (selectborder_elements.length) {
			...
			selectborder_elements.splice(start_index | 0);
		}
		this._selectborder_elements = selectborder_elements;
	};
